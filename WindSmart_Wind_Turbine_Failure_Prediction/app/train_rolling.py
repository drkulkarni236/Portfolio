...
df_x = df[['Timestamp','Turbine_ID'] + x_cols + ['status','logs_Remark','fail_Remark','block_id']]
...
# rolling avgs
df_x = df_x_0.copy()
df_x[x_cols] = df_x.groupby(['Turbine_ID'])[x_cols].rolling(window=4, min_periods=1).mean().values
...
log_likelihood = pipe.score_samples(x)
